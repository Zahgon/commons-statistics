...
public final class FDistribution extends AbstractContinuousDistribution {
    ...
    private FDistribution(double numeratorDegreesOfFreedom, double denominatorDegreesOfFreedom) {
        ...
        nHalfLogNmHalfLogM = nhalf * Math.log(numeratorDegreesOfFreedom) + mhalf * Math.log(denominatorDegreesOfFreedom);
        ...
        if (denominatorDegreesOfFreedom > MIN_DENOMINATOR_DF_FOR_VARIANCE) {
            ...
            variance = (2 * (denominatorDegreesOfFreedom * denominatorDegreesOfFreedom) * (numeratorDegreesOfFreedom + denominatorDegreesOfFreedom - 2)) / (numeratorDegreesOfFreedom * (denomDFMinusTwo * denomDFMinusTwo) * (denominatorDegreesOfFreedom - 4));
        } else {
            variance = Double.NaN;
        }
    }
    ...
    private double computeDensity(double x, boolean log) {
        ...
        double p;
        if (nx > m) {
            p = y * RegularizedBeta.derivative(m / z, m / 2, n / 2);
        } else {
            p = y * RegularizedBeta.derivative(nx / z, n / 2, m / 2);
        }
        ...
        p = nHalfLogNmHalfLogM + (n / 2 - 1) * Math.log(x) - logBetaNhalfMhalf - ((n + m) / 2) * Math.log(z);
